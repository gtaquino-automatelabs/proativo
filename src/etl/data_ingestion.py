...
import logging
import asyncio
# import schedule  # Temporariamente desabilitado - módulo não instalado
import time
...
class DataIngestionOrchestrator:
    ...
    def remove_job(self, job_name: str) -> None:
        ...
        # schedule.clear(job_name)  # Temporariamente desabilitado
    ...
    def _schedule_job(self, job: IngestionJob) -> None:
        ...
        # Temporariamente desabilitado - módulo schedule não instalado
        logger.warning(f"Agendamento temporariamente desabilitado para job: {job.name}")
        """
    ...
    def stop(self) -> None:
        ...
        # Limpa agendamentos
        # schedule.clear()  # Temporariamente desabilitado
    
    def _scheduler_loop(self) -> None:
        """Loop principal do scheduler."""
        while self.running:
            try:
                # schedule.run_pending()  # Temporariamente desabilitado
                time.sleep(1)
    ...
    def create_upload_ingestion_job(self, name: str = "upload_processor", 
                                   upload_dir: Path = None,
                                   schedule_expression: str = "*/5") -> IngestionJob:
        """Cria job de ingestão específico para diretório de uploads.
        
        Args:
            name: Nome do job (default: "upload_processor")
            upload_dir: Diretório de uploads (default: data/uploads)
            schedule_expression: Expressão de agendamento (default: a cada 5 minutos)
        ...
        if upload_dir is None:
            upload_dir = Path(os.getenv('UPLOAD_DIRECTORY', 'data/uploads'))
        ...
        job = IngestionJob(
            ...
            schedule_expression=schedule_expression,
            auto_detect=True,  # Auto-detecta tipo de dados
            file_pattern="*",  # Processa todos os arquivos válidos
            move_processed=True,  # Move arquivos processados
            archive_path=processed_dir  # Para subdiretório processed
        )
        
        self.add_job(job)
        logger.info(f"Job de upload criado: {name} monitorando {upload_dir}")
        return job 