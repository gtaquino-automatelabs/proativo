...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        if not is_connected:
            logger.error("Falha na verificação da conexão com banco de dados")
        ...
        logger.error(f"Erro durante inicialização da aplicação: {str(e)}", exc_info=True)
        raise
    ...
    try:
        ...
        # TODO: Limpar outros recursos
        
    except Exception as e:
        logger.error(f"Erro durante shutdown da aplicação: {str(e)}", exc_info=True)
...
def create_app() -> FastAPI:
    ...
    if not settings.is_development():
        # Em produção, usar hosts específicos + containers Docker
        allowed_hosts = [
            "api.proativo.com", 
            "localhost", 
            "127.0.0.1",
