...
class APIClient:
    ...
    def upload_file(self, 
        ...
        Args:
            ...
            file_type: Tipo do arquivo (equipment, maintenance ou None para auto-detectar)
            description: Descrição opcional
            overwrite_existing: Se deve sobrescrever dados existentes
            
        Returns:
            Tupla (resposta_upload, sucesso)
        """
        ...
        def _upload_file():
            ...
            response, success = self.http_service.post_file(
                "/api/v1/files/upload",
            ...
            if success:
                self._update_endpoint_stats("upload_requests")
                return response
            return None
    ...
    def get_upload_history(self, limit: int = 10) -> Tuple[Dict, bool]:
        ...
        Args:
            limit: Número máximo de uploads para retornar
        ...
        def _get_history():
            response, success = self.http_service.get("/api/v1/files/history", params=params)
            if success:
                return response
            return {"uploads": [], "total_count": 0}
        
        result, success = safe_api_call(
            _get_history,
            fallback_response={"uploads": [], "total_count": 0},
