...
from components.file_upload import render_file_upload_page, render_upload_notifications_in_sidebar
...
def criar_sidebar():
    ...
    with st.sidebar:
        ...
        # Notificações de upload na sidebar
        if API_SERVICES_AVAILABLE and "api_client" in st.session_state:
            render_upload_notifications_in_sidebar(st.session_state.api_client)
...
def main():
    ...
    if st.session_state.pagina_atual == "consultas":
        pagina_consultas()
    elif st.session_state.pagina_atual == "upload":
