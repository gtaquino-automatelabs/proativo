...
class FeedbackSystem:
    ...
    def send_feedback(self,
        ...
        try:
            ...
            # Converte rating para valores esperados pelo endpoint
            rating_value = 5 if rating == "positive" else (1 if rating == "negative" else 3)
            helpful = rating == "positive"
            
            # Obtém session_id do estado da sessão ou cria um novo
            session_id = st.session_state.get("session_id", str(uuid.uuid4()))
            if "session_id" not in st.session_state:
                st.session_state.session_id = session_id
            
            # Cria cliente API
            api_client = create_api_client(self.api_base_url)
            
            # Envia feedback usando o cliente API
            success, message = api_client.send_feedback(
            ...
            if success:
                ...
                st.error(f"❌ Erro ao enviar feedback: {message}")
                return False
